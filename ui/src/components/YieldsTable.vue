<template>
  ...
  <div class="overflow-x-auto">
    ...
    <div v-else-if="!store.initializing" class="flex flex-col items-center py-16 gap-4 text-center">
      ...
      <div>
        <p class="text-sm font-medium text-gray-300">{{ t('yields.empty', { year: store.currentYear }) }}</p>
        <p class="text-xs text-gray-500 mt-1">{{ t('yields.emptyHint') }}</p>
      </div>
      <button
        @click="emit('add')"
        class="px-4 py-2 bg-blue-600 hover:bg-blue-500 text-sm font-medium rounded-md transition-colors"
      >
        {{ t('common.addEntry') }}
      </button>
    </div>
